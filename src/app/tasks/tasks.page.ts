...
export class TasksPage implements OnInit {
  ...
  orders: any =[
    {
      ...
      "status": "Picked",
      "time": "8:00 AM - 9:00 PM",
      "address": "Plot D, 27 Gizri Rd, Block 9 Clifton, Karachi, Karachi City, Sindh 75600, Pakistan",
  ...
  ngOnInit() {
    ...
    this.orders.forEach((order: any) => {
      const matchingType: any = this.orderTypes.find((type: any) => type.type === order.status);
  ...
  navigateToNextPage(item: any) {
   ...
   console.log(item)
    this.router.navigate(['/task-location'], { replaceUrl: false, state: {item: item} });
